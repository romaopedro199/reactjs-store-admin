...
export const performanceChartOptions = (theme: Theme) => {
  return {
    responsive: true,
    plugins: {
      legend: {
      ...
      tooltip: {
        ...
        external: (context) => {
          ...
          dataPoints.forEach((point) => {
            const { dataset, formattedValue } = point;

            innerHtml += `
              <div style="display: flex; align-items: center; gap: 8px; padding-top: 10px; white-space: nowrap;">
                <span style="width: 10px; height: 10px; background: ${dataset.backgroundColor}; border-radius: 50%; display: inline-block;"></span>
